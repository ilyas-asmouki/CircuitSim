...
void Resistor::stamp(Eigen::MatrixXd& conductance, Eigen::VectorXd& sources, double timestep) {
    double g = 1 / resistance;
    if (node1 != 0) conductance(node1-1, node1-1) += g;
    if (node2 != 0) conductance(node2-1, node2-1) += g;
    if (node1 != 0 && node2 != 0) {
        conductance(node1-1, node2-1) -= g;
        conductance(node2-1, node1-1) -= g;
    }
}